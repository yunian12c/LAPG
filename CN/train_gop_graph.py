...
def main() -> None:
    ...
    parser.add_argument("--hubert-dir", type=Path, default=_root / "features" / "hubert_feature")
    parser.add_argument("--wav2vec-dir", type=Path, default=_root / "features" / "tencent_wav2vec2_feature")
    parser.add_argument("--wavlm-dir", type=Path, default=_root / "features" / "wavlm_feature")
    parser.add_argument(
        "--qwen-dir",
        type=Path,
        default=_root / "features" / "qwen_feature_14layer",
        help="Qwen3-ASR frame features (L14 layernorm→1024d)",
    )
    parser.add_argument(
        "--qwen-fusion",
        type=str,
        ...
        help="film (this snapshot): SSL base → Prosody FiLM → Qwen FiLM; concat/xattn kept for compat",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=_root / "exp" / "seed42",
    )
    parser.add_argument("--f0-cache-dir", type=Path, default=_root / "caches" / "hubert_f0")
    parser.add_argument("--energy-cache-dir", type=Path, default=_root / "caches" / "hubert_energy")
    ...
    parser.add_argument("--fa-cache-dir", type=Path, default=_root / "caches" / "fa_hubert_native")
    ...
    if qwen_dir is not None:
        ...
        if args.qwen_fusion == "concat":
            ...
            if args.qwen_fusion == "film":
                print(
                    f"[feat] ssl_prosody_qwen_film: SSL {feat_dims} base → Prosody FiLM "
                    f"→ Qwen {qwen_dim}-d FiLM",
                    flush=True,
                )
        ...
        json.dump(
            {
                **vars(args),
                ...
                "qwen_fusion": args.qwen_fusion,
                "feat_fusion": (
                    "ssl_prosody_qwen_film"
                    if args.qwen_fusion == "film" and qwen_dim > 0
                    else (f"ssl_qwen_{args.qwen_fusion}" if qwen_dim > 0 else "ssl_only")
                ),
                "recipe": (
                    "gop_graph_v102b_ssl_prosody_qwen_film"
                    if args.qwen_fusion == "film"
                    else f"gop_graph_v102b_qwen_{args.qwen_fusion}"
                ),
