...
def _fa_mask_for_graph(batch: Batch, frame_off: int, t_len: int, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
...
def _apply_fa_bridge_per_graph(
    ...
    for gid in range(int(batch.num_graphs)):
        ...
        if bool(nm.any()) and bool(fm.any()):
            fa_mask = _fa_mask_for_graph(batch, frame_off, t_len, node_h.device, node_h.dtype).unsqueeze(0)
            nh, fh = bridge(
