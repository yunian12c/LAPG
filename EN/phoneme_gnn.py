...
class PhonemeGraphModelV102b(nn.Module):
    """v102b: FA-gated 4L SSL pyramid + stem detach/match + L4 ini/fin SupCon."""
    ...
    def _encode_frames(self, batch: Batch) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.qwen_fusion == "film" and self.frame_qwen_film is not None and self._has_frame_qwen(batch):
            base_q = self.frame_qwen_film(base, batch.frame_qwen)
            base_q_fin = self.frame_qwen_film_fin(base, batch.frame_qwen)
        elif self.qwen_fusion == "xattn" and self.frame_qwen_xattn is not None and self._has_frame_qwen(batch):
            base_q = self._apply_frame_qwen_xattn(batch, base, fin=False)
            base_q_fin = self._apply_frame_qwen_xattn(batch, base, fin=True)
        else:
            base_q = base_q_fin = base
        prosody = batch.frame_prosody
        frame_h = self.frame_prosody_film(base_q, prosody)
        frame_h_fin = self.frame_prosody_film_fin(base_q_fin, prosody)
